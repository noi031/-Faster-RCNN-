...
class Rcnn_Deeplab_with_resize(nn.Module):
    ...
    def forward(self, images, image_size, seg_size, targets):
        ...
        # images = [F.interpolate(img[None], image_size, mode = 'bilinear')[0] for img in images]
...
class model_wrapper():
    def __init__(self, args, train_size, test_size, seg_size, pretrained = True):
        ...
        model = Rcnn_Deeplab_with_resize(det_seg_resnet('resnet50',
                                    ...
                                    num_rcnn_stages = 3, box_iou_thresh = [0.5, 0.6, 0.7]))
    ...
    def get_loss(self, inputs):
        ...
        loss_show = {'r_c': loss[0].item(), 'r_r': loss[1].item(),
                    'b_c': loss[2].item(), 'b_r': loss[3].item(),
                    's': loss[4].item(), 'f': loss[5].item()}
        loss = loss.sum()
        if self.model.training:
            loss.backward()
        return loss_show
...
if __name__ == '__main__':
    ...
    trainset = trainDataset(['noise', 'normalize'], '/mnt/nfs2/ikcest/data/train')
    devset = trainDataset(['normalize'], '/mnt/nfs2/ikcest/data/dev')
    trainloader = DataLoader(trainset, batch_size = 2, shuffle = True, num_workers = 2,
                            collate_fn = collate_fn)
    devloader = DataLoader(devset, batch_size = 2, num_workers = 2, collate_fn = collate_fn)
    train_wrapper = Trainprocess(model, trainloader, devloader)
    if os.path.exists('model'):
        shutil.rmtree('model')
    os.mkdir('model')
    #train_wrapper.train(0)
    train_wrapper.train(30, 3e-4)