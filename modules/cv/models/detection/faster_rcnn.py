    ...
    def assign_targets_to_anchors(self, anchors: List[Tensor],
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                labels_per_image = (matched_idxs >= 0).to(dtype = torch.int64)
...
class RoIHeads(nn.Module):
    ...
    def __init__(self, box_roi_pool, box_head, box_predictor,
                box_iou_thresh, stage_weights, bbox_reg_weights, score_thresh,
    ...
    def assign_targets_to_proposals(self, proposals: List[Tensor], targets: List[Dict[str, Tensor]],
        ...
        for proposal_per_image, targets_per_image in zip(proposals, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                labels_per_image = torch.full((proposal_per_image.shape[0],), -1,
                                    dtype = torch.int64, device = device)
    ...
    def postprocess_detections(self, class_logits: Tensor, bbox_regression: Tensor, proposals: List[Tensor],
        ...
        pred_scores_list = class_logits.split(boxes_per_image)
    ...
    def fastrcnn_loss(self, class_logits: Tensor, box_regression: Tensor,
        ...
        box_regression = box_regression.reshape(box_regression.shape[0], box_regression.shape[-1] // 4, 4)
        box_loss = self.l1_loss(
    ...
    def cascadercnn_loss(self, class_logits: Tensor, box_regression: Tensor, labels: List[Tensor],
        regression_targets: List[Tensor], weights: List[Tensor]) -> Tuple[Tensor, Tensor]:
        labels, weights = torch.cat(labels), torch.cat(weights)
        regression_targets = torch.cat(regression_targets)
        sampled_pos_inds, sampled_inds = labels >= 0, labels >= -1
        pos_num = sampled_pos_inds.sum()
        gt_labels = torch.zeros_like(class_logits).scatter_(1, labels.clamp(0)[:, None], 1)
        gt_labels[labels <= -1, :] = 0.0
        cls_loss = self.focal_loss(class_logits[sampled_inds], gt_labels[sampled_inds])
        ...
        cls_loss = torch.sum(torch.mean(cls_loss / positive, -1) * weights[sampled_inds])
        box_loss = self.l1_loss(
            box_regression[sampled_pos_inds], regression_targets[sampled_pos_inds])
        box_loss = torch.sum(box_loss.sum(-1) * weights[sampled_pos_inds]) / max(1, pos_num)
        return cls_loss, box_loss
    ...
    def forward(self, image_size: Tuple[int, int], features: List[Tensor],
        ...
        class_list, box_list = [], []
        for i, (proposal_matcher, box_head, box_predictor) in enumerate(zip(self.proposal_matcher,
                                self.box_head, self.box_predictor)):
            ...
            class_logits, box_regression = box_predictor(box_head(box_features))
            if i + 1 != len(self.box_head):
                proposals = self.box_coder.decode(box_regression, proposals)[:, 0].split(boxes_per_image)
                if targets is not None:
                    proposals = [torch.cat((box_ops.clip_boxes_to_image(proposal, image_size),
                                target['boxes'])) for proposal, target in zip(proposals, targets)]
                    proposals = [proposal[box_ops.remove_small_boxes(proposal, 1e-2)]
                                for proposal in proposals]
                else:
                    proposals = [box_ops.clip_boxes_to_image(proposal, image_size) for proposal in proposals]
                    proposals = [torch.relu(proposal - 1e-2) + 1e-2 for proposal in proposals]
            if targets is not None:
                if i + 1 < len(self.box_head):
                    loss_classifier, loss_box_reg = self.cascadercnn_loss(class_logits,
                                    box_regression, labels, regression_targets, weights)
                else:
                    loss_classifier, loss_box_reg = self.fastrcnn_loss(class_logits, box_regression,
                                        labels, regression_targets, weights)
                class_list.append(loss_classifier * self.stage_weights[i])
                box_list.append(loss_box_reg * self.stage_weights[i])
            else:
                class_list.append(class_logits)
        if targets is not None:
            result = {"loss_classifier": sum(class_list), "loss_box_reg": sum(box_list)}
        else:
            class_logits = sum([cls.sigmoid() for cls in class_list]) / len(class_list)
...
class FasterRCNN(nn.Module):
    ...
    Args:
        ...
        stage_weights (List[float]): weights for different stages.
    ...
    def __init__(self, out_channels, num_classes, num_rcnn_stages = 1,
                ...
                box_iou_thresh = [0.5], stage_weights = [1], bbox_reg_weights = None):
        ...
        if box_predictor is None:
            box_predictor = [CascadeRCNNPredictor(1024, num_classes) for _ in range(num_rcnn_stages - 1)]
        ...
        assert num_rcnn_stages == len(stage_weights)
        roi_heads = RoIHeads(
            box_roi_pool, box_head, box_predictor,
            box_iou_thresh, stage_weights, bbox_reg_weights, box_score_thresh,
